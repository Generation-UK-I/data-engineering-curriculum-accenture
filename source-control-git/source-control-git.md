...
The `exercises/git-settings.sh` and `exercises/git-settings.ps1` (windows users) files can do this in one go. Remind learners to change the `user.name` and `user.email` values before running the command.
...
Run `git config --list` to view the configurations for a repository.

> Run this now to check that the configurations from the previous slide have been set correctly.

If this command is run outside of a git repository it will show the global git configuration settings.

Notes:
Get everyone to run this to check what their config is
...
_Make sure you are in the correct folder before you do the "clone"!_

Notes:
Make sure everyone is in the correct "C:\sot-de-<month>-<year>" folder!
