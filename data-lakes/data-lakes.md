...
- Identify a typical AWS architecture
...
Remote Storage (S3 for example) became:
...
**Hidden spiraling costs** - Always price and confirm your work (e.g. https://calculator.s3.amazonaws.com/index.html).
...
- Data availability that is too open and leaking privacy data ([There are many famous s3 leaks](https://arstechnica.com/information-technology/2017/05/defense-contractor-stored-intelligence-data-in-amazon-cloud-unprotected))
...
In this example Kafka will inject data from a bunch of different data sources.
...
- AWS Kinesis Streams/Firehose for ingestion
- AWS S3 for storage
- AWS Glue for Data Cataloguing (Glue can also perform ETL for you)
- AWS Athena for querying and processing

Notes:
We can do everything in the last slide within the AWS ecosystem.

AWS Kinesis Streams/Firehose can ingest and move data from within and outside the AWS ecosystem.

This will land in an S3 bucket, which functions as a cloud based data store.

The most effective way to catalog data for later use is with AWS Glue.

Once the data is stored and catalogued Athena allows us to process and query it for analysis.
...
![](img/data-lake-architecture.gif)<!-- .element: class="centered" -->

Notes:

- Page view data is sent directly to a Kinesis Firehose
- The Firehose outputs data raw into an s3 bucket
- An AWS Glue crawler creates a catalogue of the data in this bucket, and another bucket detailing user device Data
- An AWS Glue ETL jobs runs against these two buckets on a schedule and outputs data into a further bucket for pre-processed views
- The same raw page view Data is also sent to Kinesis Analytics where it is filtered, eventually landing in another s3 bucket
- Data is queried using AWS Athena and visualised using Amazon Quicksight
